...
def plot_frozenlake_results(algo_name, Q_table, env, gamma, test_reward, train_reward=None, v=None, savefig=False):
    ...
    if train_reward:
        ...
        ax_list[0, 0].plot(train_reward, lw=0.3)
        average_train_reward = np.mean(train_reward)
        ax_list[0, 0].axhline(y=average_train_reward, color='red', lw=1)
        if average_train_reward > 0.1:
            ax_list[0, 0].set_yticks([0, average_train_reward, 1])
        else:
            ax_list[0, 0].set_yticks([average_train_reward, 1])
    else:
    ...
    ax_list[0, 1].plot(test_reward, lw=0.3)
    ax_list[0, 1].set_title('Testing')

    average_test_reward = np.mean(test_reward)
    ax_list[0, 1].axhline(y=average_test_reward, color='red', lw=1)
    ax_list[0, 1].set_ylim([0, 1.1])
    ax_list[0, 1].set_yticks([0, average_test_reward, 1])
