...
class ModelFreeAgent:
    ...
    def policy(self, state, is_training):
        ...
        if is_training and np.random.random() < self.eps:
    ...
    def train_step(self, state, action, reward, next_state, next_action, done):
        ...
        if self.algorithm == RLAlgorithm.SARSA:
            ...
            expected_q = self.eps * np.mean(q_next) + (1.0 - self.eps) * np.max(q_next)
            next_q = 0.0 if done else expected_q
            td_target = reward + self.gamma * next_q
            self.Q[state, action] += self.alpha * (td_target - self.Q[state, action])
...
BEST_HP = {
    (RLAlgorithm.SARSA,          0.95): (0.05, 0.9993),
    (RLAlgorithm.Q_LEARNING,     0.95): (0.10, 0.9990),
    (RLAlgorithm.EXPECTED_SARSA, 0.95): (0.10, 0.9993),
    (RLAlgorithm.SARSA,          1.0):  (0.10, 0.9997),
    (RLAlgorithm.Q_LEARNING,     1.0):  (0.05, 0.9993),
    (RLAlgorithm.EXPECTED_SARSA, 1.0):  (0.10, 0.9997),
}
    ...
    for gamma in [0.95, 1.0]:
        for algo in [RLAlgorithm.SARSA, RLAlgorithm.Q_LEARNING, RLAlgorithm.EXPECTED_SARSA]:
            ...
            train_test_agent(algorithm=algo, gamma=gamma, alpha=alpha, eps=eps, eps_decay=eps_decay,
                             ...
                             max_episode_length=200, savefig=True, seed=0)
