...
def train_my_logic():
    ...
    print("--- Start Training: Hybrid (GMM Raw + XGBoost Scaled) ---")
    ...
    print("1. Trainiere Outlier Detector (GMM unskaliert)...")
    ...
    print(f"   -> Threshold Tau (Raw GMM): {tau:.4f}")

    # TEIL 2: KLASSIFIKATION
    print("2. Trainiere Classifier (XGBoost skaliert)...")
    scaler = StandardScaler()
    X_train_scaled = scaler.fit_transform(X_train_raw)
    
    scores_train = gmm_model.score_samples(X_train_raw)
    mask_clean = scores_train >= tau
    
    X_train_clean_scaled = X_train_scaled[mask_clean]
    y_train_clean = y_train[mask_clean]
    
    print(f"   -> Trainiere XGBoost auf {len(X_train_clean_scaled)} sauberen Samples...")
...
def predict(X_test):
    ...
    labels = xgb_model.predict(features_scaled).astype(int)
...
def main():
    ...
    try:
        ...
        print(f"-> Check: {n_out} Outliers im Leaderboard-Set gefunden.")
