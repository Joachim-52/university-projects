...
def compute_design_matrix(data: np.ndarray) -> np.ndarray:
    ...
    N = data.shape[0]
    ones = np.ones((N, 1))
    design_matrix = np.hstack([ones, data])
...
def multiple_loss(X: np.ndarray, y: np.ndarray, theta: np.ndarray) -> float:
    ...
    multiple_loss = np.mean((y-y_pred)**2)

    return multiple_loss
...
def compute_polynomial_design_matrix(x: np.ndarray, K: int) -> np.ndarray:
    ...
    polynomial_design_matrix = np.ones((N, 1))

    # add columns for x^1, x^2
    for k in range(1, K +1):
        # calculate x^k and reshape it to a column vector (N, 1)
        x_k = (x**k).reshape(N, 1)
        polynomial_design_matrix = np.concatenate((polynomial_design_matrix, x_k), axis=1)
