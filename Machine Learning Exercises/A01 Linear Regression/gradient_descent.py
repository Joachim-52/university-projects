...
def gradient_descent(f: Callable[[np.ndarray, np.ndarray], np.ndarray], 
    ...
    # Initialize current point and learning rate
    x_curr = x0
    y_curr = y0
    eta_t = learning_rate
    
    # Run gradient descent for num_iters
    for t in range(1, num_iters + 1):
        # Calculate gradient
        gradient = df(x_curr, y_curr)
        df_dx = gradient[0]
        df_dy = gradient[1]
        
        # Update parameters
        x_curr = x_curr - eta_t * df_dx
        y_curr = y_curr - eta_t * df_dy
        
        # Store new values
        x_list[t] = x_curr
        y_list[t] = y_curr
        f_list[t] = f(x_curr, y_curr)
        
        # Update learning rate
        eta_t = eta_t * lr_decay
